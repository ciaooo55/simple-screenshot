...
class SettingsDialog(QDialog):
    ...
    def __init__(
        ...
        note = QLabel(
            ...
            "F1 查看全部快捷键。<br>"
            "<b>完成</b>：松开框选直接打开可最小化、可从任务栏切换的截图会话窗口；"
            "窗口内默认画笔，首层可切箭头/矩形，更多菜单保留荧光、椭圆、序号、马赛克、文字；"
            "左键拖动超过系统阈值直接绘制，双击执行默认复制/保存并关闭；"
            "滚轮只缩放图片，不移动窗口。Ctrl+C 复制、Ctrl+S 保存；"
            "W 后只显示原图文字选择层，识别完成可拖选文字，"
            "Ctrl+A 全选、Ctrl+C 复制、Esc 或右键返回画笔；"
            "右键依次取消当前笔迹、撤销最后一笔、回到框选、取消截图。<br>"
            "<b>贴图</b>：无边框置顶；默认左键画笔，按住中键才拖动（贴边自动吸附），Ctrl+拖动拖出为文件，"
            "滚轮以光标为中心缩放，Ctrl+滚轮调透明度，方向键微调，"
            "双击或 Ctrl+0 还原；右键可撤销/暂停画笔、开鼠标穿透（托盘恢复）或识别文字，"
