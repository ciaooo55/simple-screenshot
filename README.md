...
- **W 识别选区文字（OCR）**：识别结果弹出可编辑面板，一键复制全部；中英混排自动排版。识别前自动放大小字号文字（13px 文字准确率实测 81%→96%），并使用去掉箭头/荧光等标注的干净图（马赛克保留，打码内容不会泄漏进识别结果）。使用 Windows 系统自带识别引擎，完全离线、不增加体积，需要系统装有中文/英文语言包（中文系统默认已有）。贴图右键菜单也有"识别文字"。
