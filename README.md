...
5. 选区确定后自动切换到画笔，直接在框内拖动就能涂画标注。**双击选区即可完成**（执行本次默认动作：复制/保存/钉住）——画笔/形状工具下双击只要没拖出笔迹（3px 抖动容差内）就会完成，拖动了则照常作画；序号/文字工具例外：序号连击是快速放序号，文字工具双击打开输入框。任何工具下也可按 Enter 完成，或点击工具栏中的"钉住 / 复制 / 保存"临时切换。右键逐级返回：先退回框选，再次右键才退出。
