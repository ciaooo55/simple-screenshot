...
5. 松开框选后立刻打开**截图预览**：像普通图片窗口一样可最小化，图片在固定画布中居中，滚轮只缩放图片而不移动窗口；顶部只保留复制、保存、识别、画笔四个操作。双击执行本次快捷键的默认动作（Alt+A 截图时复制，Alt+S 截图时保存）并自动关闭预览；点“画笔”才进入完整标注。识别文字时画笔栏自动隐藏，退出识别后恢复。
...
截图预览点“画笔”或右键选择“画笔标注”后才进入此模式；这样常用截图只需框选、双击，复杂标注仍随时可用。

工具栏从左到右：选择、画笔、荧光、箭头、矩形、椭圆、序号、马赛克、文字。编辑状态下可用字母键快速切换工具：**V** 选择、**P** 画笔、**G** 荧光、**A** 箭头、**R** 矩形、**O** 椭圆、**N** 序号、**M** 马赛克、**T** 文字（数字键亦可）。

- 进入“标注编辑”后自动切换到画笔，直接就能涂画；按 **F1**（或 H）随时呼出快捷键一览面板。
- 画笔：自由绘制（贝塞尔平滑，笔迹无折角），可选 8 种颜色与 2/4/6/8 像素粗细；**滚轮直接调粗细**，文字工具下滚轮调字号。
- 荧光笔：半透明加粗高亮，划过文字不遮挡内容，适合标重点；颜色粗细与画笔共用。
...
- 文字：点击选区后输入；Ctrl+Enter 完成文字，普通 Enter 可换行；输入框随内容自动加高，渲染位置、换行与输入框所见完全一致，自带衬底阴影保证浅色背景可读。**输入过程中可直接点工具栏换颜色/字号，新样式实时应用到输入框**，不会误提交。
...
- Ctrl+C 立即复制，Ctrl+S 立即保存，**Ctrl+Shift+S 另存为**（弹出对话框自选路径和文件名，会记住上次另存位置）；Ctrl+D 立即钉住。
- **W 识别选区文字（OCR）**：按下后图片固定在原位，识别过程鼠标键盘自动穿透，可继续操作聊天、浏览器等其他程序；结果回来后可像普通文本一样拖动选择，双击选词，Ctrl+A 全选，Ctrl+C 复制，Esc 返回截图编辑。识别使用去掉箭头/荧光等标注的干净图（马赛克保留，打码内容不会泄漏进识别结果）。内置高精度识别引擎（PaddleOCR 模型，与微信同类技术，完全离线），推理运行在独立进程，显卡驱动或原生引擎异常不会带崩截图主程序；贴图窗口同样支持按 W 在原图上选择文字。
- 右键逐级回退：先撤销最后一笔标注；没有标注时取消当前框选以便重新框选；尚未框选时退出截图。Esc 的分级行为相同；拖到一半时只取消当前这一笔/这次选区变换，文字输入时只关闭输入框。
