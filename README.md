...
- **W 识别选区文字（OCR）**：识别结果弹出可编辑面板，一键复制全部；中英混排自动排版。识别使用去掉箭头/荧光等标注的干净图（马赛克保留，打码内容不会泄漏进识别结果）。内置高精度识别引擎（PaddleOCR 模型，与微信同类技术，完全离线），有显卡时自动 GPU 加速（实测约 0.7 秒/张，11px 小字准确率 96%）；引擎异常时自动回退 Windows 系统识别。贴图右键菜单也有"识别文字"。
