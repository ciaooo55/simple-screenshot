...
6. 右键托盘图标可手动截图（复制 / 保存 / 钉住 / 全屏）、保存最近一张截图、关闭所有贴图、打开截图文件夹、打开设置、切换开机启动或退出。
...
- 画笔：自由绘制，可选 8 种颜色与 2/4/6/8 像素粗细。
...
- 选区不满意时，可直接单击另一个窗口重新吸附，或在阴影区域拖出新框；新选区会替换旧选区（重选后保留你选中的标注工具）。右键逐级返回：先关文字框，再取消拖到一半的一笔，再退回框选，最后才退出截图。
...
- 贴图上 Ctrl+C 复制（有"已复制"提示）、Ctrl+S 保存、Esc 关闭；右键菜单显示当前缩放/透明度，还可一键关闭所有贴图（托盘菜单同样可以）。
- 可同时钉住多张贴图，互不影响。
