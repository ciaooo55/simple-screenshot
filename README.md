...
5. 松开框选后立刻打开**截图预览**：像普通图片窗口一样可最小化，图片在固定画布中居中，滚轮只缩放图片而不移动窗口；顶部只保留复制、保存、识别、编辑四个操作。双击执行本次快捷键的默认动作（Alt+A 截图时复制，Alt+S 截图时保存）并自动关闭预览；只有需要画笔、箭头、马赛克等功能时才进入完整编辑，默认不打断工作。
...
截图预览右键选择“标注编辑”后才进入此模式；这样常用截图只需框选、双击，复杂标注仍随时可用。
...
- Esc 取消截图；拖到一半时按 Esc 只取消当前这一笔/这次选区变换，不会整个退出。文字输入时 Esc 只关闭输入框。
