...
- **W 识别选区文字（OCR）**：识别后仍停留在原图，可像普通文本一样拖动选择；双击选词，Ctrl+A 全选，Ctrl+C 复制，Esc 返回截图编辑。识别使用去掉箭头/荧光等标注的干净图（马赛克保留，打码内容不会泄漏进识别结果）。内置高精度识别引擎（PaddleOCR 模型，与微信同类技术，完全离线），推理运行在独立进程，显卡驱动或原生引擎异常不会带崩截图主程序；贴图窗口同样支持按 W 在原图上选择文字。
