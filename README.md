...
5. 松开框选后打开普通的**截图会话窗口**：它贴合截图内容，仅保留窄边，显示在任务栏、可最小化/最大化且不置顶，不影响其他应用。窗口内默认就是画笔，左键拖动超过系统拖拽距离才落笔；双击且位移很小时执行本次快捷键的默认动作（Alt+A 复制，Alt+S 保存）并关闭窗口。滚轮只缩放图片。W 识别文字后可拖选复制，画笔操作会暂时隐藏，退出识别后恢复。
